...
class amber_a100(Amber_test):
    ...
    def setting_variables(self):
     ...
     if self.variant == "a100_1":
        ...
        self.executable_opts = ['-O', '-i mdin.GPU', '-o mdout', '-p JAC.prmtop', '-c JAC.inpcrd', '-r restrt', '-x mdcrd']
        self.reference = {
            'ibex:batch': {
                'Elapsed_time': (1050.00, -0.05, None, 'ns/day')
                }
            } 
        ...
        self.reference = {
            'ibex:batch': {
                'Elapsed_time': (1250.00, -0.05, None, 'ns/day')
             }
            }
        ...
        self.reference = {
            'ibex:batch': {
                'Elapsed_time': (1300.00, -0.05, None, 'ns/day')
             }
            }
...
class amber_v100(Amber_test):
    ...
    def setting_variables(self):
     ...
     if self.variant == "v100_1":
        ...
        self.reference = {
            'ibex:batch': {
                'Elapsed_time': (900.00, -0.05, None, 'ns/day')
                }
            } 
        ...
        self.reference = {
            'ibex:batch': {
                'Elapsed_time': (1100.00, -0.05, None, 'ns/day')
             }
            }
        ...
        self.reference = {
            'ibex:batch': {
                'Elapsed_time': (1000.00, -0.05, None, 'ns/day')
             }
            }
    ...
    def set_job_options(self):
        if self.variant == "v100_1":
           self.job.options = ['--constraint=v100',
                              '--gpus=1',
                              '--cpus-per-task=4',
                              '--mem=50G'
                              ]
...
class amber_cpu(Amber_test):
    ...
    def setting_variables(self):
     ...
     self.executable_opts = ['-O', '-i mdin.CPU', '-o mdout', '-p JAC.prmtop', '-c JAC.inpcrd', '-r restrt', '-x mdcrd']
     self.reference = {
            'ibex:batch': {
                'Elapsed_time': (100.00, -0.1, None, 'ns/day')
                }
            } 
