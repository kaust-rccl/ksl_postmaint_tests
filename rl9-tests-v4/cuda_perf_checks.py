...
class Cuda_perf_checks(rfm.RegressionTest):
    """
    Define base class for CUDA performance test.
    """
    ...
    def setting_variables(self):
        ...
        self.perf_patterns = {
            self.variant: sn.extractsingle(
                r"Performance:\s+(?P<Gflops>\S+) Gflop/s", self.stdout, "Gflops", float
            )
        }
        self.reference = {
            "ibex": {
                "p100": (6.6, -0.1, None, "Gflops"),
