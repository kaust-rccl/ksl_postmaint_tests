...
class Cuda_power_checks(rfm.RunOnlyRegressionTest):
      ...
      def setting_variables(self):
        ...
        self.tags = {'gpu',self.variant,'acceptance','power_query','cuda','singlenode'}
        # Environment settings
        self.valid_systems = ['ibex:batch']

        self.valid_prog_environs = ['gpustack_cuda']
        self.time_limit = '10m'
        if self.variant == 'v100_8' or self.variant == 'rtx2080ti' or self.variant == 'a100_8' :
        ...
        self.executable='bash -c \'nvidia-smi -q -d POWER | grep -i "Current Power Limit" | grep -v "N/A"\''
        ...
        # Performance check
        power_limits = sn.extractall(r'Current Power Limit\s+:\s+(?P<watts>\d+\.\d+)\sW', self.stdout, 'watts', float) 
        min_power = sn.min(power_limits)
        self.perf_patterns = {self.variant : min_power }
        self.reference = {
                            'ibex' : {      'p100': (250.0,None,None,'watts'),
