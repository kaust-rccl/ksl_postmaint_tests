...
# The Path to source code and method of compilation will be required
@rfm.simple_test
class gromacs_tests(rfm.RunOnlyRegressionTest):
    """
    Define base class for GROMACS CPU test.
    """
    ...
    def setting_variables(self):
        ...
        # This will be appended to the resulting srun command line.
        ...
        # Check https://reframe-hpc.readthedocs.io/en/latest/reference.html
        ...
        self.sanity_patterns = sn.assert_found(
            r"\s+:-\) GROMACS - gmx mdrun, 2023 \(-:", self.stderr, encoding="utf-8"
        )
        self.perf_patterns = {
            self.variant: sn.extractsingle(
                r"^Performance:\s+(?P<ns_day>\S+)", self.stderr, "ns_day", float
            )
        }
