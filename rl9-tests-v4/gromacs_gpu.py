...
class gromacs_gpu(rfm.RunOnlyRegressionTest):
    """
    Define base class for GROMACS GPU test.
    """
...
class gromacs_a100(gromacs_gpu):
    ...
    def setting_variables(self):
        ...
        if self.variant == "a100_1":
            ...
            self.executable = (
                "gmx_mpi mdrun -deffnm topol -nb gpu -pme gpu "
                "-update gpu -bonded gpu -nsteps 100000 -resetstep 90000 "
                "-noconfout -dlb no -nstlist 300 -pin on"
            )
            self.reference = {
                "ibex:batch": {"Elapsed_time": (324, -0.1, None, "ns/day")}
            }
            ...
            self.executable = (
                "mpirun -np 2 gmx_mpi mdrun -deffnm topol -nb gpu -pme gpu "
                "-npme 1 -update gpu -bonded gpu -nsteps 100000 -resetstep 90000 "
                "-noconfout -dlb no -nstlist 300 -pin on"
            )
            self.reference = {
                "ibex:batch": {"Elapsed_time": (370, -0.1, None, "ns/day")}
            }
