...
class pytorch_test(rfm.RunOnlyRegressionTest):
    """
    Define base class for pytorch tests.
    """
...
class pytorch_a100_gpu(pytorch_test):
    """
    Define base class for pytorch A100 GPUs tests.
    """
...
class pytorch_v100_gpu(pytorch_test):
    """
    Define base class for pytorch V100 GPUs tests.
    """
    ...
    def set_job_options(self):
        ...
        if self.variant == "v100_8_singlenode":
            ...
            self.job.options = ["--gpus=16", "--gpus-per-node=4"]
        elif self.variant == "v100_1_multinode":
            self.job.options = ["--gpus=2", "--gpus-per-node=1"]


@rfm.simple_test
class pytorch_rtx4090_gpu(pytorch_test):
    """
    Define base class for pytorch RTX4090 GPUs tests.
    """

    variant = parameter(["rtx4090_singlegpu"])
    time_limit = "3h"
    reference = {"ibex": {"rtx4090_singlegpu": (8951, None, 0.1, "Epoch_time")}}
    ...
    def set_job_options(self):
        """
        Deine extra test resources.
        """
        if self.variant == "rtx4090_singlegpu":
            self.job.options = ["--gpus=1", "--gpus-per-node=1"]
