...
class slurm_check(rfm.RunOnlyRegressionTest):
    """
    Define base class for slurm checks.
    """
    ...
    def setting_variables(self):
        """
        Define test resources and variants.
        """
        ...
        if self.variant == "slurmctld":
            ...
            self.sanity_patterns = sn.assert_found(
                r"JobID           JobName  Partition    Account  AllocCPUS      State ExitCode ",
                self.stdout,
            )

        self.maintainers = ["mohsin.shaikh@kaust.edu.sa"]
        self.tags = {"slurm", "acceptance", "cpu", "singlenode"}
