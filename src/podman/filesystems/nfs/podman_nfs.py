...
BASE_DIR = os.path.join(os.environ["HOME"], "podman_test")
NFS_ROOT = os.path.join(BASE_DIR, "root")
NFS_RUN = os.path.join(BASE_DIR, "run")

# Create necessary directories
os.makedirs(NFS_RUN, exist_ok=True)

# 3) Diagnostics
hostname = socket.gethostname()
print(f"\n Job is running on: {hostname}")
print(f" Podman root      = {NFS_ROOT}")
print(f" Podman runroot   = {NFS_RUN}")

# Check filesystem type (should show 'nfs' in Type column if NFS)
try:
    df_output = subprocess.check_output(["df", "-T", BASE_DIR], text=True)
    print("Filesystem check (expecting 'nfs'):")
